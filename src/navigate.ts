...
function startNavigation(inRoute: Array<string>) {
  ...
  let firstRoute = routes.filter(
    (x) => x.from == inRoute[0] && x.to == inRoute[1]
  )[0];

  if (firstRoute?.mode == "MRT") {
    let firstProvider = providers.filter(
      (x) => x.name == firstRoute?.provider
    )[0];
    ...
    speakText(
      "Proceed to " +
        firstPlace.id +
        ", " +
        (firstPlace.displayName ?? firstPlace.longName)
    );
  ...
  route.forEach((placeId, i) => {
    ...
    let possibleRoutes = routes.filter(
      (x) => x.from == placeId && x.to == route[i + 1]
    );
    ...
    if (possibleRoutes.length == 0) {
      if (from.type == "MRT" && to.type == "MRT") {
        ...
        let nextRoute = routes.filter(
          (x) => x.from == to.id && x.to == route[i + 2]
        )[0];
        ...
        let transferToProvider = providers.filter(
          (x) => x.name == nextRoute.provider
        )[0];
      ...
      if (from.type == "airport" && to.type == "airport") {
        ...
        let phrase =
          "Welcome to " +
          (to.displayName ?? to.longName ?? to.shortName) +
          "... ";
...
function navigationLoop() {
  ...
  fetch(
    `https://thingproxy.freeboard.io/fetch/https://dynmap.minecartrapidtransit.net/standalone/dynmap_new.json`
  )
    .then((response) => {
