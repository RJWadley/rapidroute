...
//export a firebase function that will load data every day at midnight UTC
import * as functions from "firebase-functions";
export const loadDataFunction = functions.pubsub
  .schedule("0 0 * * *")
  .onRun(loadData);
...
const handoffData = async (
  ...
  routes.map((x) => {
    ...
    let routeId = makeSafe(`${x.provider}-${x.number ?? placeA + placeB}`);
    ...
    let routesWithSameNumber = routes.filter(
      (y) => y.number === x.number && y.provider === x.provider
    );
  ...
  const routesByLocationRef = ref(database, "routesByLocation");
  snapshot = await get(routesByLocationRef);
  let routesByLocationFromDB: Record<string, RouteByLocation> =
    snapshot.exists() ? snapshot.val() : {};

  for (let key in routesByLocationFromDB) {
    if (routesByLocationFromDB[key].autoGenerated === true) {
      refsToRemove["routesByLocation/" + key] = true;
      delete routesByLocationFromDB[key];
    } else console.log("Route " + key + " is not auto generated");
  }

  //group routes by location
  let routesByLocation: Record<string, string[]> = {};
  routes.forEach((x) => {
    let placeA = x.from > x.to ? x.to : x.from;
    let placeB = x.from > x.to ? x.from : x.to;
    let routeId = makeSafe(`${x.provider}-${x.number ?? placeA + placeB}`);

    routesByLocation[makeSafe(x.from)] =
      routesByLocation[makeSafe(x.from)] ?? [];
    if (!routesByLocation[makeSafe(x.from)].includes(routeId)) {
      routesByLocation[makeSafe(x.from)].push(routeId);
    }
    routesByLocation[makeSafe(x.to)] = routesByLocation[makeSafe(x.to)] ?? [];
    if (!routesByLocation[makeSafe(x.to)].includes(routeId)) {
      routesByLocation[makeSafe(x.to)].push(routeId);
    }
  });

  for (let key in routesByLocation) {
    let routeByLocation: RouteByLocation = {
      autoGenerated: true,
      routes: routesByLocation[key],
    };

    if (!routesByLocationFromDB[key]) {
      refsToRemove["routesByLocation/" + key] = false;
      set(ref(database, "routesByLocation/" + key), routeByLocation);
    } else console.log("Route " + key + " already exists in database");
  }

  console.log("routesByLocation set");
