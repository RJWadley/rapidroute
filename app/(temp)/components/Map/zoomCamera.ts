...
export const zoomToPlayer = (x: number, z: number, viewport: Viewport) => {
	...
	const distance = Math.hypot(currentCenter.x - x, currentCenter.y - z)
	if (distance > 3000)
...
export const zoomToPoint = async (
	point: Point,
	viewport: Viewport,
	boxSize = 500,
): Promise<void> => {
	await zoomToTwoPoints(
		new Point(point.x + boxSize, point.y + boxSize),
		new Point(point.x - boxSize, point.y - boxSize),
		viewport,
	)
	// TODO
	// if (canMoveCamera()) zoomToPoint(point, viewport, boxSize).catch(console.error)
}
