...
const getDistanceOnAxis = (
	...
	if (size1 === 0 || size2 === 0) return Number.POSITIVE_INFINITY
...
const getAllCullDistances = (objectsToCheck: CullInput[]): CullInfo[] => {
	...
	for (const object of sortedObjects) {
		...
		for (const existingObject of cullInfo) {
			if (!existingObject.target.bounds) continue
