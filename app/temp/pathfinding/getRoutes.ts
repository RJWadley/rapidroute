...
export function getRoutes({
	...
	while (!frontier.isEmpty()) {
		...
		edges
			...
			.filter(
				(edge) =>
					(costSoFar.get(current) ?? Number.POSITIVE_INFINITY) + edge.weight <
					maxCost,
			)
			.forEach((edge) => {
				const newCost =
				...
				if (
					...
					newCost < (costSoFar.get(toEdge) ?? Number.POSITIVE_INFINITY)
				) {
...
function updateMaxCost(
	...
	const distanceToTo = getDistance(
		currentNode.coordinate_x ?? Number.POSITIVE_INFINITY,
		currentNode.coordinate_z ?? Number.POSITIVE_INFINITY,
		destinationNode.coordinate_x ?? Number.POSITIVE_INFINITY,
		destinationNode.coordinate_z ?? Number.POSITIVE_INFINITY,
	)
