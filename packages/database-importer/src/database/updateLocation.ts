...
import { PathingPlace } from "@rapidroute/database-utils"
import { SearchIndexItem } from "@rapidroute/database-utils"

import { database } from "./database"
import deepCompare from "./deepCompare"

const isLocation = databaseTypeGuards.places
const unusedKeys: Record<string, string[]> = {}

export function setLocation(
  locationId: string,
  location: Place | undefined | null
) {
  ...
  const thisLocation = database.places[locationId]
  const previousLocation: Place | undefined = thisLocation
    ? {
        ...thisLocation,
        uniqueId: locationId,
      }
  ...
  if (
    previousLocation &&
    location &&
    !previousLocation.autoGenerated &&
  ...
  if (location) database.places[locationId] = location
  else delete database.places[locationId]
  ...
  if (location) {
    const newValue: SearchIndexItem = {
      ...
      i: `${location.name} ${location.shortName} ${
        Array.isArray(location.ownerPlayer)
...
export function beforeLocationUpdate(source: string | true = true) {
  ...
  const keys = Object.keys(database.places || {}).filter(
    key => database.places?.[key]?.autoGenerated === source
  )
